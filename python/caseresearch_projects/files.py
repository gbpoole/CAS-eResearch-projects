import caseresearch_projects as pkg
import caseresearch_projects.lines as lines
import os
...
def create_file(filename_out: str | Path, n_lines: int = 10) -> None:
    ...
    try:
        ...
        raise CreateFileError(f"Could not open output file {{{filename_out}}}.") from e
...
def read_file(filename_in: str | Path) -> List[str]:
    ...
    try:
        ...
        raise ReadFileError(
            f"File open/close error when reading file {{{filename_in}}}."
        ) from e
